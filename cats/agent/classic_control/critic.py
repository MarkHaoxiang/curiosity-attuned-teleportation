...
class ClassicalResetCritic(Critic):
    def __init__(self, env: gym.Env, features: int = 128) -> None:
        ...
        self.net = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Linear(
                in_features=features, out_features=2
            ),  # First index is non-truncate, second is truncate
        )
...
class ClassicalInjectionResetCritic(Critic):
    ...
    def q(self, s: Tensor, a: Tensor):
        ...
        q = torch.where(
            t <= 0.5,
            heads[..., 0],
            torch.clamp(heads[..., 1], min=0) + self.reset_value - t,
        )
        return q
